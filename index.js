...
function GitSSBWeb(ssb, config, reconnect) {
  ...
  ssb.whoami(function (err, feed) {
    this.myId = feed.id
    this.about = ssbAbout(ssb, this.myId)
  }.bind(this))
...
G.handleRequest = function (req) {
  ...
  if (dir == '')
    ...
    return this.serveUserPage(req, dir, dirs.slice(1))
  else if (dir == 'static')
    ...
    readReqForm(req, function (err, data) {
        ...
        case 'fork':
          ...
          return ssbGit.createRepo(self.ssb, {upstream: data.id},
            function (err, repo) {
              ...
              cb(null, self.serveRedirect(req, encodeLink(repo.id)))
            })
      ...
      case 'new-issue':
        ...
        return self.ssb.publish(msg, function (err, msg) {
          if (err) return cb(null, self.serveError(req, err))
          cb(null, self.serveRedirect(req, encodeLink(msg.key)))
        })
  ...
  switch (c.type) {
    ...
    case 'pull-request':
      ...
      return self.getMsg(c.project, function (err, projectMsg) {
        if (err) return cb(null, self.serveRepoNotFound(req, c.repo, err))
...
G.serveUserPage = function (req, feedId, dirs) {
  switch (dirs[0]) {
    case undefined:
    case '':
    case 'activity':
...
G.renderUserPage = function (req, feedId, page, titleTemplate, body) {
  var self = this
  return readNext(function (cb) {
    self.about.getName(feedId, function (err, name) {
      if (err) return cb(err)
      var title = titleTemplate ? titleTemplate
        .replace(/\%{name\}/g, escapeHTML(name))
        : escapeHTML(name)
      cb(null, self.serveTemplate(req, title)(cat([
        pull.once('<h2>' + link([feedId], name) +
...
G.serveUserActivity = function (req, feedId) {
  return this.renderUserPage(req, feedId, 'activity', null,
    this.renderFeed(req, feedId))
}

G.serveUserRepos = function (req, feedId) {
  var self = this
  var title = req._t('UsersRepos', {name: '%{name}'})
  return this.renderUserPage(req, feedId, 'repos', title, pull(
    cat([
      self.ssb.messagesByType({
        type: 'git-update',
        reverse: true
      }),
      self.ssb.messagesByType({
    ...
    paramap(function (msg, cb) {
      ...
      var done = multicb({ pluck: 1, spread: true })
      self.getRepoName(feedId, repoId, done())
      self.getVotes(repoId, done())
      ...
      switch (c.type) {
        case 'git-repo':
          return self.getRepo(id, function (err, repo) {
            if (err) return cb(null, self.serveError(req, err))
            cb(null, self.serveRepoPage(req, Repo(repo), path))
          })
        case 'git-update':
          return self.getRepo(c.repo, function (err, repo) {
            if (err) return cb(null, self.serveRepoNotFound(req, c.repo, err))
            cb(null, self.serveRepoUpdate(req, Repo(repo), id, msg, path))
          })
        case 'issue':
          return self.getRepo(c.project, function (err, repo) {
            if (err) return cb(null,
              self.serveRepoNotFound(req, c.project, err))
            self.issues.get(id, function (err, issue) {
              if (err) return cb(null, self.serveError(req, err))
              cb(null, self.serveRepoIssue(req, Repo(repo), issue, path))
            })
          })
        case 'pull-request':
          return self.getRepo(c.repo, function (err, repo) {
            ...
            self.pullReqs.get(id, function (err, pr) {
              if (err) return cb(null, self.serveError(req, err))
              cb(null, self.serveRepoPullReq(req, Repo(repo), pr, path))
            })
          })
        case 'issue-edit':
          if (ref.isMsgId(c.issue)) {
            return self.pullReqs.get(c.issue, function (err, issue) {
              ...
              var serve = issue.msg.value.content.type == 'pull-request' ?
                self.serveRepoPullReq : self.serveRepoIssue
              self.getRepo(issue.project, function (err, repo) {
                if (err) {
                  if (!repo) return cb(null,
                    self.serveRepoNotFound(req, c.repo, err))
                  return cb(null, self.serveError(req, err))
                }
                cb(null, serve.call(self, req, Repo(repo), issue, path, id))
              })
            })
          }
          // fallthrough
                ...
                switch (root.content && root.content.type) {
                  case 'issue':
                    return self.issues.get(c.root, function (err, issue) {
                      if (err) return cb(null, self.serveError(req, err))
                      return cb(null,
                        self.serveRepoIssue(req, Repo(repo), issue, path, id))
                    })
                  case 'pull-request':
                    return self.pullReqs.get(c.root, function (err, pr) {
                      if (err) return cb(null, self.serveError(req, err))
                      return cb(null,
                        self.serveRepoPullReq(req, Repo(repo), pr, path, id))
                    })
                }
              })
            })
        ...
        default:
          if (ref.isMsgId(c.repo))
            return self.getRepo(c.repo, function (err, repo) {
              if (err) return cb(null,
                self.serveRepoNotFound(req, c.repo, err))
              cb(null, self.serveRepoSomething(req, Repo(repo), id, msg, path))
            })
          else
            return cb(null, self.serveGenericMessage(req, id, msg, path))
      }
...
G.serveRepoPage = function (req, repo, path) {
  var self = this
  ...
  if (query.rev != null) {
    ...
    return self.serveRedirect(req, url.format(req._u))
  }

  // get branch
  return path[1] ?
    G_serveRepoPage2.call(self, req, repo, path) :
    readNext(function (cb) {
      ...
      repo.getSymRef('HEAD', true, function (err, ref) {
        ...
        repo.resolveRef(ref, function (err, rev) {
          path[1] = rev ? ref : null
          cb(null, G_serveRepoPage2.call(self, req, repo, path))
        })
      })
    })
}

function G_serveRepoPage2(req, repo, path) {
  var branch = path[1]
      ...
      switch (path[1]) {
        case 'new':
          if (filePath.length == 0)
            return this.serveRepoNewIssue(req, repo)
          break
        default:
          return this.serveRepoIssues(req, repo, false)
      }
    case 'pulls':
      return this.serveRepoIssues(req, repo, true)
    case 'compare':
      return this.serveRepoCompare(req, repo)
    case 'comparing':
      return this.serveRepoComparing(req, repo)
    default:
      return this.serve404(req)
  }
}

G.serveRepoNotFound = function (req, id, err) {
  return this.serveTemplate(req, req._t('error.RepoNotFound'), 404)
  (pull.values([
...
G.renderRepoPage = function (req, repo, page, branch, titleTemplate, body) {
  var self = this
  ...
  var done = multicb({ pluck: 1, spread: true })
  self.getRepoName(repo.feed, repo.id, done())
  self.about.getName(repo.feed, done())
  self.getVotes(repo.id, done())

  if (repo.upstream) {
    self.getRepoName(repo.upstream.feed, repo.upstream.id, done())
    self.about.getName(repo.upstream.feed, done())
  }
  ...
  return readNext(function (cb) {
      ...
      if (err) return cb(null, self.serveError(req, err))
      var upvoted = votes.upvoters[self.myId] > 0
...
G.serveEmptyRepo = function (req, repo) {
  if (repo.feed != this.myId)
...
G.serveRepoTree = function (req, repo, rev, path) {
...
G.serveRepoActivity = function (req, repo, branch) {
  var self = this
  ...
  return self.renderRepoPage(req, repo, 'activity', branch, title, cat([
    ...
    readOnce(function (cb) {
      var done = multicb({ pluck: 1, spread: true })
      self.about.getName(repo.feed, done())
      self.getMsg(repo.id, done())
      done(function (err, authorName, msg) {
        if (err) return cb(err)
        self.renderFeedItem(req, {
          key: repo.id,
          value: msg,
...
G.serveRepoCommits = function (req, repo, branch) {
  var query = req._u.query
...
G.serveRepoCommit = function (req, repo, rev) {
  var self = this
  return readNext(function (cb) {
    repo.getCommitParsed(rev, function (err, commit) {
      if (err) return cb(err)
      var commitPath = [repo.id, 'commit', commit.id]
      var treePath = [repo.id, 'tree', commit.id]
      var title = escapeHTML(commit.title) + ' · ' +
...
G.serveRepoSomething = function (req, repo, id, msg, path) {
...
G.serveRepoUpdate = function (req, repo, id, msg, path) {
  var self = this
  ...
  var commits = cat([
    msg.content.objects && pull(
      ...
      paramap(function (obj, cb) {
        self.getBlob(req, obj.link || obj.key, function (err, readObject) {
          if (err) return cb(err)
    ...
    msg.content.packs && pull(
      ...
      paramap(function (pack, cb) {
        var done = multicb({ pluck: 1, spread: true })
        self.getBlob(req, pack.pack.link, done())
        self.getBlob(req, pack.idx.link, done())
...
G.serveRepoBlob = function (req, repo, rev, path) {
  var self = this
  return readNext(function (cb) {
    repo.getFile(rev, path, function (err, object) {
      if (err) return cb(null, self.serveBlobNotFound(req, repo.id, err))
      var type = repo.isCommitHash(rev) ? 'Tree' : 'Branch'
      var pathLinks = path.length === 0 ? '' :
        ': ' + linkPath([repo.id, 'tree'], [rev].concat(path))
...
G.serveRepoRaw = function (req, repo, branch, path) {
  return readNext(function (cb) {
    repo.getFile(branch, path, function (err, object) {
      if (err) return cb(null,
        this.serveBuffer(404, req._t('error.BlobNotFound')))
      var extension = getExtension(path[path.length-1])
      var contentType = imgMimes[extension]
      cb(null, pull(object.read, this.serveRaw(object.length, contentType)))
    })
  })
}
...
G.serveRepoDigs = function (req, repo) {
  var self = this
  return readNext(function (cb) {
  var title = req._t('Digs') + ' · %{author}/%{repo}'
    self.getVotes(repo.id, function (err, votes) {
...
G.getForks = function (repo, includeSelf) {
  var self = this
  return pull(
    cat([
      includeSelf && readOnce(function (cb) {
        self.getMsg(repo.id, function (err, value) {
    ...
    paramap(function (msg, cb) {
      self.getRepoFullName(msg.value.author, msg.key,
...
G.serveRepoForks = function (req, repo) {
  var hasForks
...
G.serveRepoForkPrompt = function (req, repo) {
...
G.serveRepoIssues = function (req, repo, isPRs) {
  var self = this
  var count = 0
  ...
  return self.renderRepoPage(req, repo, page, null, title, cat([
    pull.once(
      (self.isPublic ? '' :
    ...
    pull(
      (isPRs ? self.pullReqs : self.issues).list({
        repo: repo.id,
        project: repo.id,
...
G.serveRepoNewIssue = function (req, repo, issueId, path) {
  var title = req._t('issue.New') + ' · %{author}/%{repo}'
  return this.renderRepoPage(req, repo, 'issues', null, title, pull.once(
...
G.serveRepoIssue = function (req, repo, issue, path, postId) {
  var self = this
  var isAuthor = (self.myId == issue.author) || (self.myId == repo.feed)
  var newestMsg = {key: issue.id, value: {timestamp: issue.created_at}}
  var title = escapeHTML(issue.title) + ' · %{author}/%{repo}'
  return self.renderRepoPage(req, repo, 'issues', null, title, cat([
    pull.once(
      renderNameForm(req, !self.isPublic, issue.id, issue.title, 'issue-title',
    ...
    readOnce(function (cb) {
      self.about.getName(issue.author, function (err, authorName) {
        if (err) return cb(err)
    ...
    self.isPublic ? pull.empty() : readOnce(function (cb) {
...
G.renderIssueActivityMsg = function (req, repo, issue, type, postId, msg) {
  var authorLink = link([msg.value.author], msg.authorName)
  ...
  switch (c.type) {
    case 'post':
      if (c.root == issue.id) {
        var changed = this.issues.isStatusChanged(msg, issue)
        return '<section class="collapse">' +
    ...
    case 'issue-edit':
      return '<section class="collapse">' +
        ...
        '</section>'
    case 'git-update':
      var mention = this.issues.getMention(msg, issue)
      if (mention) {
...
G.serveRepoPullReq = function (req, repo, pr, path, postId) {
  var self = this
  var headRepo, authorLink
  var page = path[0] || 'activity'
  var title = escapeHTML(pr.title) + ' · %{author}/%{repo}'
  return self.renderRepoPage(req, repo, 'pulls', null, title, cat([
    pull.once('<div class="pull-request">' +
      renderNameForm(req, !self.isPublic, pr.id, pr.title, 'issue-title', null,
    ...
    readOnce(function (cb) {
      ...
      self.about.getName(pr.author, done())
      var sameRepo = (pr.headRepo == pr.baseRepo)
      self.getRepo(pr.headRepo, function (err, headRepo) {
        if (err) return cb(err)
        self.getRepoName(headRepo.feed, headRepo.id, done())
        self.about.getName(headRepo.feed, done())
...
G.renderPullReqCommits = function (req, pr, baseRepo, headRepo, cb) {
  var self = this
  self.pullReqs.getRevs(pr.id, function (err, revs) {
...
G.renderPullReqFiles = function (req, pr, baseRepo, headRepo, cb) {
  this.pullReqs.getRevs(pr.id, function (err, revs) {
    if (err) return cb(null, renderError(err))
    cb(null, cat([
      pull.once('<section>'),
      renderDiffStat(req,
...
G.renderPullReqActivity = function (req, pr, repo, headRepo, authorLink, postId) {
  var self = this
  ...
  var isAuthor = (self.myId == pr.author) || (self.myId == repo.feed)
  return cat([
    readOnce(function (cb) {
      cb(null,
        '<section class="collapse">' +
    ...
    pull(
      ...
      pull.map(function (item) {
        ...
        return self.renderIssueActivityMsg(req, repo, pr,
          req._t('pull request'), postId, item)
      })
    ),
    !self.isPublic && isAuthor && pr.open && pull.once(
      ...
      '</pre>' +
      '</div></section>'),
    !self.isPublic && readOnce(function (cb) {
...
G.renderBranchUpdate = function (req, pr, msg) {
  var authorLink = link([msg.value.author], msg.authorName)
  var msgLink = link([msg.key],
    new Date(msg.value.timestamp).toLocaleString(req._locale))
...
G.serveRepoCompare = function (req, repo) {
  var self = this
  var query = req._u.query
  var base
  var count = 0
  ...
  return self.renderRepoPage(req, repo, 'pulls', null, title, cat([
    ...
    pull(
      self.getForks(repo, true),
      pull.asyncMap(function (msg, cb) {
        self.getRepo(msg.key, function (err, repo) {
          if (err) return cb(err)
          cb(null, {
            msg: msg,
            repo: repo
...
G.serveRepoComparing = function (req, repo) {
  var self = this
  var query = req._u.query
  ...
  if (!s || !baseBranch)
    return self.serveRedirect(req, encodeLink([repo.id, 'compare']))
  ...
  return self.renderRepoPage(req, repo, 'pulls', null, pageTitle, cat([
    pull.once('<h3>' + title + '</h3>'),
    readNext(function (cb) {
      self.getRepo(headRepoId, function (err, headRepo) {
        if (err) return cb(err)
        self.getRepoFullName(headRepo.feed, headRepo.id,
          function (err, repoName, authorName) {
            if (err) return cb(err)
...
G.renderCommitLog = function (req, baseRepo, baseBranch, headRepo, headBranch) {
  return cat([
